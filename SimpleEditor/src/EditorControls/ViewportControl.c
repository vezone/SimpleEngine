...
force_inline void
editor_camera_settings()
{
    if (g_IsCameraSettingsOpen)
    {
	if (igBegin("Camera Settings", &g_IsCameraSettingsOpen, ImGuiWindowFlags_None))
	{
	    if (igButton("Perspective", ImVec2_(0.0f, 0.0f)))
	    {
		ImGuizmo_SetOrthographic(false);
		editor_camera_set_perspective(g_Camera);
	    }
	    if (igButton("Orthographic", ImVec2_(0.0f, 0.0f)))
	    {
		ImGuizmo_SetOrthographic(true);
		editor_camera_set_orthograhic(g_Camera);
	    }
	    ...
	    igSliderFloat3("Position", &g_Camera->Position, -25.f, 25.f, "%0.1f", ImGuiSliderFlags_None);
	}
	igEnd();
    }
}
...
force_inline void
guizmo(Scene* scene, Entity selectedEntity, Camera* camera)
{
    ImVec2 windowPos;
    f32 width;
    f32 height;
	...
	{
	    v3_assign_xyz(snap, 1.0f, 1.0f, 1.0f);
	}
	else if (g_OperationType == ROTATE)
	{
	    v3_assign_xyz(snap, 5.0f, 5.0f, 5.0f);
	}
	else if (g_OperationType == SCALE)
	{
	    v3_assign_xyz(snap, 0.1f, 0.1f, 0.1f);
	}
    }

    ImGuizmo_Manipulate((f32*)camera->View,
			(f32*)camera->Projection,
			g_OperationType, LOCAL,
			(f32*)&transformComponent->Transform[0][0],
			NULL, useSnap ? (f32*)&snap[0] : NULL, NULL, NULL);
}

void
viewport_create(NativeWindow window, EditorCamera* editorCamera, FrameBuffer* framebuffer)
{
    g_Window = window;
...
viewport(Scene* scene, Camera* camera)
{
	...
	v2_assign_xy(g_Bounds[0], minBounds.x + windowPos.x, minBounds.y + windowPos.y);
	v2_assign_xy(g_Bounds[1], maxBounds.x + windowPos.x, maxBounds.y + windowPos.y);
